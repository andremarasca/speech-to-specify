...
class MockProvider:
    ...
    def complete(self, prompt: str) -> str:
        ...
        elif "tasks" in prompt.lower() or "tarefas" in prompt.lower() or "decomposição" in prompt.lower():
            return self._mock_tasks(prompt_hash)
...
"""
    ...
    def _mock_tasks(self, hash_suffix: str) -> str:
        """Generate a mock tasks artifact."""
        return f"""# Tasks [{hash_suffix}]

## Preparação e Alicerce

A estratégia de inicialização começa pela criação da estrutura de diretórios e 
configuração do ambiente de desenvolvimento. A base técnica inclui a instalação 
de dependências e configuração do sistema de build.

## O Caminho do MVP

A funcionalidade principal será construída incrementalmente, começando pelos 
modelos de dados e progredindo para a camada de persistência. Cada commit 
representa uma unidade lógica completa e reversível.

## Evolução e Expansão

As funcionalidades subsequentes serão integradas de forma incremental, 
respeitando as dependências estabelecidas na fase anterior. Testes automatizados 
garantem a estabilidade do sistema.

## Consolidação e Refinamento

O processo final inclui otimização de performance, revisão de segurança e 
validação global. Documentação técnica acompanha cada entrega.
