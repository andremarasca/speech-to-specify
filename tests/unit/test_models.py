...
class TestArtifact:
    ...
    def test_invalid_step_name_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Artifact(
                execution_id="exec-123", step_number=1, step_name="invalid_step", content="content"
            )
    ...
    def test_empty_content_rejected(self):
        ...
        with pytest.raises(ValidationError):
            Artifact(execution_id="exec-123", step_number=1, step_name="constitution", content="")

    def test_get_filename(self):
        """Test filename generation."""
        artifact = Artifact(
            execution_id="exec-123", step_number=2, step_name="specification", content="content"
        )
    ...
    def test_step_number_must_be_positive(self):
        ...
        with pytest.raises(Exception):
            Artifact(
                execution_id="exec-123", step_number=0, step_name="constitution", content="content"
            )
...
class TestExecution:
    ...
    def test_failed_requires_error_message(self):
        ...
        with pytest.raises(ValidationError):
            Execution(input_id="input-123", status=ExecutionStatus.FAILED, error_message=None)
...
class TestArtifactMetadataCompleteness:
    """Tests verifying artifact metadata is self-documenting (US5)."""

    def test_artifact_has_all_required_metadata(self):
        """Test that artifact has all required metadata fields."""
        artifact = Artifact(
            execution_id="exec-123",
            step_number=2,
            step_name="specification",
            predecessor_id="pred-456",
            content="# Specification\n\nSome content",
        )

        # Required for self-documentation
        assert artifact.id is not None
        assert artifact.execution_id is not None
        assert artifact.step_number is not None
        assert artifact.step_name is not None
        assert artifact.created_at is not None
        assert artifact.predecessor_id is not None  # For step > 1

    def test_artifact_first_step_no_predecessor(self):
        """Test that first step artifact has no predecessor (valid)."""
        artifact = Artifact(
            execution_id="exec-123",
            step_number=1,
            step_name="constitution",
    ...
    def test_artifact_has_timestamp(self):
        """Test that artifact created_at is auto-generated."""
        from datetime import datetime, timezone

        before = datetime.now(timezone.utc)
        artifact = Artifact(
            execution_id="exec-123", step_number=1, step_name="constitution", content="content"
        )
        after = datetime.now(timezone.utc)

        assert before <= artifact.created_at <= after

    def test_artifact_filename_zero_padded(self):
        """Test that artifact filename has zero-padded step number."""
        artifact = Artifact(
            execution_id="exec-123", step_number=1, step_name="constitution", content="content"
        )
        ...
        # Should be 01_constitution.md not 1_constitution.md
        assert filename == "01_constitution.md"
        assert filename.startswith("0")  # Zero-padded


class TestExecutionMetadataCompleteness:
    """Tests verifying execution.json contains full context (US5)."""

    def test_execution_has_input_id(self):
        """Test that execution references its input."""
        execution = Execution(input_id="input-123")

        assert execution.input_id == "input-123"

    def test_execution_has_timestamps(self):
        """Test that execution has started_at and completed_at."""
        execution = Execution(input_id="input-123")
    ...
    def test_execution_has_step_count(self):
    ...
    def test_execution_serialization_preserves_metadata(self):
        ...
        restored = Execution(**data)
        assert restored.input_id == completed.input_id
